...
from kazoo.protocol.states import KazooState, EventType, ZnodeStat

if TYPE_CHECKING:
    from kazoo.client import KazooClient, WatchFunc
    from kazoo.interfaces import IAsyncResult, Threadlike
...
class TreeCache(object):
    ...
    def __init__(self, client: KazooClient, path: str):
        ...
        self._task_thread: Threadlike | None = None
    ...
    def _publish_event(
        self, event_type: int, event_data: NodeData | None = None
    ) -> None:
    ...
    def _in_background(
        self,
        func: Callable[[str, str, IAsyncResult], None],
        method_name: str,
        path: str,
        result: IAsyncResult,
    ) -> None: ...
...
class TreeNode(object):
    ...
    def _publish_event(
        ...
        return self._tree._publish_event(event_type, event_data)
    ...
    def _process_result(
        self, method_name: str, path: str, result: IAsyncResult
    ) -> None:
...
# FIXME these Tuple-based classes would look a lot better using NamedTuple
# though the event_type in TreeEvent needs sorting.
class TreeEvent(Tuple[int, Union["NodeData", None]]):
    """The immutable event tuple of cache."""

    # FIXME These should be an enum.
    NODE_ADDED = 0
    NODE_UPDATED = 1
    NODE_REMOVED = 2
    ...
    @classmethod
    def make(
        cls, event_type: int, event_data: NodeData | None = None
    ) -> TreeEvent:
...
class NodeData(Tuple[str, bytes, ZnodeStat]):
    ...
    @classmethod
    def make(cls, path: str, data: bytes, stat: ZnodeStat) -> NodeData:
