...
class TestSSLClient:
    ...
    client cert/key/CA options (see contracts/client-connection.md), so a
    connected ``zkclient`` has already completed a real TLS handshake with a
    client certificate.
    """
...
class TestReconfig:
    ...
    def test_bad_input(self, zkensemble):
        client = zkensemble.get_client(superadmin=True)
        client.start()
        try:
            with pytest.raises(BadArgumentsError):
