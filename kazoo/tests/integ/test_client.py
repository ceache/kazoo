...
class TestSSLClient:
    ...
    def test_four_letter_word_command(self, zkclient):
        client = zkclient

        # Verify that the client is connected via mutual TLS
        assert client.use_ssl is True
        assert isinstance(client._connection._socket, ssl.SSLSocket)

        # 1. Test 4LW 'ruok' command over TLS:
        # Exercises client.command() -> create_connection() with peer_host
        response = client.command(b"ruok")
        assert response == "imok"

        # 2. Test 4LW 'srvr' command over TLS
        srvr_response = client.command(b"srvr")
        assert "Zookeeper version:" in srvr_response

        # 3. Test server_version() over TLS:
        # Calls command(b"envi") internally and parses the version tuple
