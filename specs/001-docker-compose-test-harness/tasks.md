...
- [x] T039 Run the quickstart.md validation scenarios V1–V10 across all auth flavors and feature sets on the plain + security matrix; fix any gaps found (SC-001..SC-007). Validated under a Python 3.14 venv with kazoo (editable) + testcontainers 4.15.0:
  ...
  - V5 isolation / V6 failure-injection verified by repeated queuing-runs (5/6 queuing tests pass; session-expired variant skipped above). V7 log-dump verified via the captured `dump_ensemble_logs` on failures. V8 partial: full `kazoo/tests/integ` suite green; `git grep KazooTestCase|KazooTestHarness|ZookeeperCluster` returns only docs/comment mentions (test_connection.py has commented-out legacy snippets; kazoo/testing/__init__.py docstring). V9 (Linux/macOS/Windows CI) and V10 (clean-venv `pip install -e '.[test]'`) not yet fully executed.
